...
import { about, education, experience, skills } from "@/lib/datas";
import {
  EducationList,
  ExperienceList,
  SkillsListType,
          ...
          <div className="min-h-[70vh] w-full">
            <TabsContent value="experience" className="w-full">
              <div className="flex flex-col gap-[30px] text-center xl:text-left">
                ...
                <ScrollArea className="h-[264px] mb-[16px]">
                  <ul className="grid grid-cols-1 lg:grid-cols-2 gap-[30px]">
                    {experience.experienceList.map(
                      (item: ExperienceList, index: number) => (
                        <li
                          key={index}
                ...
                <ScrollArea className="h-[264px] mb-[16px]">
                  <ul className="grid grid-cols-1 lg:grid-cols-2 gap-[30px]">
                    {education.educationList.map(
                      (item: EducationList, index: number) => (
                        <li
                          key={index}
                          className="bg-[#232329] h-[184px] py-6 px-10 rounded-xl flex flex-col justify-center items-center lg:items-start gap-1"
                        >
              ...
              <div className="flex flex-col gap-[30px]">
                ...
                <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 xl:gap-[30px]">
                  {skills.skillsList.map(
                    (Item: SkillsListType, index: number) => (
                      <li key={index}>
            ...
            >
              <div className="flex flex-col gap-[30px]">
                ...
                <ul className="grid grid-cols-1 gap-y-6 max-w-[620px] mx-auto xl:mx-0">
                  {about.info.map((item: AboutInfoType, index: number) => (
                    <li
                      key={index}
